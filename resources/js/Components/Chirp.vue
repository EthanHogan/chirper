        ...
        <div class="font-extrabold flex item-center justify-between mt-0.5 mb-1.5">
            ...
            <div class="relative rounded-full cursor-pointer hover:bg-gray-880">
                ...
                <div v-if="openOptions" class="absolute mt-1 p-3 right-0 w-[300px] bg-black border border-gray-700 rounded-lg shadow-lg">
                    <Link as="button" method="delete" :href="route('chirps.destroy', { id: chirp.id })" class="flex items-center cursor-pointer">
                        <TrashCanOutline class="pr-3" fillColor="#DC2626" :size="18" />
                        <span class="font-extrabold text-red-600">Delete</span>
                    </Link>
                </div>
            </div>
        </div>
